...
class QuizApiController extends Controller
{
     ...
     * @return JsonResponse
     */
    public function checkAnswerer($locale, Request $request): JsonResponse
    {
        ...
        if (isset($mobile) && !$isRetryAllow) {
            $quizResult = QuizResult::where('mobile', '=', $mobile)->where('quiz_id', '=', $quizId)->first();           
        ...
        if(isset($email) && !$isRetryAllow){
            $quizResult = QuizResult::where('email', '=', $email)->where('quiz_id', '=', $quizId)->first();
            if(isset($quizResult?->id)) {
                return response()->json([
                    'message' => 'You have already participated in this quiz. Please wait for the winner announcement',
                    'isValid' => false
     ...
     * @return JsonResponse
     */
    public function createAnswererAccount($locale, Request $request): JsonResponse
    {
        App::setLocale($locale);
     ...
     * @return JsonResponse
     */
    public function submitQuestionAnswer($locale, Request $request): JsonResponse
    {
        App::setLocale($locale);
