...
use App\Jobs\Child\ReadyDOBNotification;
use App\Jobs\Child\ReadyDOBPushNotification;
use App\Notifications\Schedule\SendChildDOBDBNotification;
...
class AddChildDOBNotification implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if($notificationPeriod === 'after') {
            ...
            Child::whereDate('date_of_birth', $date)
                        ...
                        if($child?->parent_id) {
                            ...
                            if($customer?->id) {
                                ...
                                if($notificationType === 'email' && $customer?->email) {  
                                    ReadyDOBNotification::dispatch($customer, $template, $child);
                                }
                                ...
                                if($notificationType === 'push_notification') {
                                    ...
                                    if($commentHelpers?->isSettingEnabled($customer, 'push_notification', 'all')){
                                        ...
                                        if(count($fcmTokens)){
                                            ReadyDOBPushNotification::dispatch($customer, $template, $child, $fcmTokens);
                                        }
                                    }
                                }
                                ...
                                if($notificationType === 'database') {
                                    ...
                                    $customer?->notify((new SendChildDOBDBNotification($customer, $template, $child)));                   
                                }
                            }
